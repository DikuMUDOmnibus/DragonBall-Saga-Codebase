...
void do_practice( CHAR_DATA * ch, char *argument )
{
  ...
  if( is_reploid( ch ) )
    {
    send_to_char( "Reploids cannot practice, they must seek Dr. Light for their weapons.\n\r", ch );
    return;
  }
  ...
  if( ( !str_cmp( argument, "mechaicer" ) || !str_prefix( argument, "mechaicer" ) ) && argument[0] != '\0' )
  {
    ch_printf( ch, "You must seek Mechanization.\n\r" );
    return;
  }
  ...
  else
  {
    ...
    if( skill_table[sn]->min_level[ch->class] == 0
        ...
        || ( sn == gsn_wss && !IS_DEMONRANKED( ch ) )
        || ( sn == gsn_bpoint && !IS_SET( ch->pcdata->flags, PCFLAG_KNOWSLSSJ ) )
        || ( sn == gsn_super_dragonfist && ch->race == race_lookup( "saiyan" ) && !SAIYANRANKED( ch ) )
        || ( sn == gsn_finalshine && ch->race == race_lookup( "saiyan" ) && ch->rank < 2 )
        || ( sn == gsn_manifestpotara && ch->race == race_lookup( "kaio" ) && ch->rank < 6 )
        || ( sn == gsn_deathbeam && ch->race == race_lookup( "icer" ) && ch->rank < 2 ) )
    {
    ...
    if( ch->race == race_lookup( "bio-android" ) )
    {
      ...
      else if( sn == gsn_ki_burst || sn == gsn_clone || sn == gsn_negative_spirit_bomb )
      {
        bioPrac = TRUE;
      }
      else
...
//  bool    canMystic = FALSE;
  ...
  for( sn = gsn_first_ability; sn < gsn_first_weapon; sn++ )
  {
    if( ( skill_table[sn]->guild != CLASS_NONE
          && ( !IS_GUILDED( ch ) || ( ch->pcdata->clan->class != skill_table[sn]->guild ) ) ) )
      continue;
    if( sn == gsn_wss && !IS_DEMONRANKED( ch ) )
      continue;
    if( sn == gsn_bpoint && !IS_SET( ch->pcdata->flags, PCFLAG_KNOWSLSSJ ) )
      continue;
    if( sn == gsn_finalshine && ch->race == race_lookup( "saiyan" ) )
      if( ch->rank < 2 )
        continue;
    ...
    if( sn == gsn_suppress && is_android( ch ) && ch->pcdata->learned[sn] < 1 )
      continue;
    if( sn == gsn_manifestpotara && ( ch->pcdata->learned[sn] < 1 || !IS_KAIO( ch ) ) )
      continue;
    if( sn == gsn_deathbeam && ( ch->pcdata->learned[sn] < 1 || !IS_ICER( ch ) ) )
      continue;
 ...
 pager_printf_color(ch, "&O-----------&WInventory&O-----------------&WExp Needed&O-&WCurrent&O---&WMax&O---\n\r");
else if (is_reploid(ch))
 pager_printf_color(ch, "&C-----------&BWeapons&C-----------------&BPower Needed&C-&BCurrent&C---&BMax&C---\n\r");
