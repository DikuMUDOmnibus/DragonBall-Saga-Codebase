...
/* See if char can do dual-wielding */
#define COULD_DUAL(ch) ( IS_NPC( ch ) || ch->pcdata->learned[gsn_dual_wield] )
...
void wear_obj( CHAR_DATA * ch, OBJ_DATA * obj, bool fReplace, sh_int wear_bit )
{
                                         ...
                                         !( IS_OBJ_STAT( obj, ITEM_ANTI_EVIL ) && IS_EVIL( ch ) ) ) ) )
  {
    if( !IS_OBJ_STAT( obj, ITEM_ANTI_ANDROID ) ||
        ...
        ( !IS_OBJ_STAT( obj, ITEM_ANTI_NAMEK ) &&
          is_namek( ch ) ) ||
        ( !IS_OBJ_STAT( obj, ITEM_ANTI_KAIO ) &&
          IS_KAIO( ch ) &&
          IS_ARCHANGEL( ch ) &&
          IS_NEPHALIM( ch ) ) ||
        ...
        ( !IS_OBJ_STAT( obj, ITEM_ANTI_SYMBIOTE ) &&
          is_symbiote( ch ) ) ||
        ( !IS_OBJ_STAT( obj, ITEM_ANTI_TUFFLE ) && 
          is_tuffle( ch ) ) ||
	 ( !IS_OBJ_STAT( obj, ITEM_ANTI_FIERIAN ) &&
     	   is_fierian( ch ) ) ||
	 ( !IS_OBJ_STAT( obj, ITEM_ANTI_HYLIAN ) &&
     	   is_hylian( ch ) ) ||
	 ( !IS_OBJ_STAT( obj, ITEM_ANTI_REPLOID ) &&
     	   is_reploid( ch ) )||
	 ( !IS_OBJ_STAT( obj, ITEM_ANTI_MUTANT ) &&
     	   IS_MUTANT( ch ) ) )
    {
      bypass = TRUE;
    }
  ...
  if( !bypass )
    if( ( IS_OBJ_STAT( obj, ITEM_ONLY_SAIYAN ) && !is_saiyan( ch ) )
        ...
        || ( IS_OBJ_STAT( obj, ITEM_ONLY_DEMON ) && !IS_DEMON( ch ) && !IS_NEPHALIM( ch ) && !IS_ARCHDEMON( ch ) )
        || ( IS_OBJ_STAT( obj, ITEM_ONLY_KAIO ) && !IS_KAIO( ch ) && !IS_NEPHALIM( ch ) && !IS_ARCHANGEL( ch ) )
        ...
        || ( IS_OBJ_STAT( obj, ITEM_ONLY_FIERIAN ) && !is_fierian( ch ) )
        || ( IS_OBJ_STAT( obj, ITEM_ONLY_HYLIAN ) && !is_hylian( ch ) )
        || ( IS_OBJ_STAT( obj, ITEM_ONLY_REPLOID ) && !is_reploid( ch ) )
        || ( IS_OBJ_STAT( obj, ITEM_ONLY_MUTANT ) && !IS_MUTANT( ch ) )
...
//      || (IS_OBJ_STAT(obj, ITEM_ANTI_HUMAN) && is_human(ch))
        ...
        || ( IS_OBJ_STAT( obj, ITEM_ANTI_DEMON ) && ( IS_DEMON( ch ) || IS_NEPHALIM( ch ) || IS_ARCHDEMON( ch ) ) )
        || ( IS_OBJ_STAT( obj, ITEM_ANTI_KAIO ) && ( IS_KAIO( ch ) || IS_NEPHALIM( ch ) || IS_ARCHANGEL( ch ) ) )
        ...
        || ( IS_OBJ_STAT( obj, ITEM_ANTI_FIERIAN ) && is_fierian( ch ) )
        || ( IS_OBJ_STAT( obj, ITEM_ANTI_HYLIAN ) && is_hylian( ch ) )
        || ( IS_OBJ_STAT( obj, ITEM_ANTI_REPLOID ) && is_reploid( ch ) )
        || ( IS_OBJ_STAT( obj, ITEM_ANTI_MUTANT ) && IS_MUTANT( ch ) )
        || ( IS_OBJ_STAT( obj, ITEM_ANDROID_CHIP ) && ( !is_android( ch ) && !is_superandroid( ch ) && !is_reploid( ch ) ) ) )
    {
      act( AT_MAGIC, "You are forbidden to use that item.", ch, NULL, NULL, TO_CHAR );
      act( AT_ACTION, "$n tries to use $p, but is forbidden to do so.", ch, obj, NULL, TO_ROOM );
      return;
  ...
  switch ( 1 << bit )
  {
    ...
    case ITEM_WEAR_TAIL:
      if( !remove_obj( ch, WEAR_TAIL, fReplace ) )
        return;
      if(!ch->pcdata || ch->pcdata->tail < 1) {
	send_to_char("You have no tail to put that on.", ch);
	return;
      }
      if( !oprog_use_trigger( ch, obj, NULL, NULL, NULL ) )
      {
        act( AT_ACTION, "$n holds $p with $s tail.", ch, obj, NULL, TO_ROOM );
        act( AT_ACTION, "You hold $p with your tail.", ch, obj, NULL, TO_CHAR );
      }
    ...
    case ITEM_WEAR_BADGE:
      ...
      if( !oprog_use_trigger( ch, obj, NULL, NULL, NULL ) )
      {
        act( AT_ACTION, "$n pins $p to $s chest.", ch, obj, NULL, TO_ROOM );
        act( AT_ACTION, "You pin $p to your chest.", ch, obj, NULL, TO_CHAR );
      }
    ...
    case ITEM_WEAR_AURA:
      ...
      if( !oprog_use_trigger( ch, obj, NULL, NULL, NULL ) )
      {
        act( AT_ACTION, "$n bursts into $p.", ch, obj, NULL, TO_ROOM );
        act( AT_ACTION, "You burst into $p.", ch, obj, NULL, TO_CHAR );
      }
    ...
    case ITEM_WEAR_NOSE:
      ...
      if( !oprog_use_trigger( ch, obj, NULL, NULL, NULL ) )
      {
        act( AT_ACTION, "$n puts $p in $s nose.", ch, obj, NULL, TO_ROOM );
        act( AT_ACTION, "You put $p in your nose.", ch, obj, NULL, TO_CHAR );
      }
    ...
    case ITEM_WEAR_FLOATING:
      ...
      if( !oprog_use_trigger( ch, obj, NULL, NULL, NULL ) )
      {
        act( AT_ACTION, "$n lets go of $p, and it begins to float about $m.", ch, obj, NULL, TO_ROOM );
        act( AT_ACTION, "You let go of $p, and it begins to float about you.", ch, obj, NULL, TO_CHAR );
      }
