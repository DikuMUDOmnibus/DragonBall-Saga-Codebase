...
#define	CD	CHAR_DATA
// CD *find_banker args( ( CHAR_DATA * ch ) );
...
/* void do_bank( CHAR_DATA *ch, char *argument )
{
  ...
  {
    total = ch->pcdata->balance;
    total += ch->gold;
    
    set_char_color( AT_GREEN, ch );
    sprintf( buf, "You are carrying %d zeni.\n\r", ch->gold );
    send_to_char( buf, ch );
    sprintf( buf, "You also have %d zeni in the bank.\n\r",
             ch->pcdata->balance );
    send_to_char( buf, ch );
    sprintf( buf, "Making a total of %d zeni.\n\r",
             total );
    send_to_char( buf, ch );
    return;
  }
  ...
  if ( !str_cmp( arg1, "deposit" ) )
  {
    ...
    if ( arg2 == '\0' )
    {
      sprintf( buf, "%s How much zeni do you wish to deposit?", ch->name );
      do_tell( banker, buf );
      return;
    }
    ...
    if ( amount > ch->gold )
    {
      sprintf( buf, "%s Sorry, but you don't have that much zeni to deposit.",
      	       ch->name );
      do_tell( banker, buf );
      return;
    }
    ...
    sprintf( buf, "You deposit %d zeni.\n\r", amount );
    set_char_color( AT_PLAIN, ch );
    send_to_char( buf, ch );
    sprintf( buf, "$n deposits %d zeni.\n\r", amount );
  ...
  if ( !str_cmp( arg1, "withdraw" ) )
  {
    ...
    if ( arg2 == '\0' )
    {
      sprintf( buf, "%s How much zeni do you wish to withdraw?", ch->name );
      do_tell( banker, buf );
      return;
    }
    if ( str_cmp( arg2, "all" ) && !is_number( arg2 ) )
    ...
    {
      sprintf( buf, "%s, you do not have that much zeni in your account!",
      	       ch->name );
      do_tell( banker, buf );
      return;
    }
    ...
    if ( amount <= 0 )
    {
      sprintf( buf, "Oh I see %s.. I didn't know i was doing business with a comedian.",
               ch->name );
      do_tell( banker, buf );
      return;
    }
    ...
    ch->pcdata->balance	-= amount;
    ch->gold		+= amount;
    sprintf( buf, "You withdraw %d zeni.\n\r", amount );
    set_char_color( AT_PLAIN, ch );
    send_to_char( buf, ch );
    sprintf( buf, "$n withdraws %d zeni.\n\r", amount );
    act( AT_PLAIN, buf, ch, NULL, NULL, TO_ROOM );
    return;
  }
  ...
  if ( !str_cmp( arg1, "transfer" ) )
  {
    ...
    if ( arg2 == '\0' || arg3 == '\0' )
    {
      sprintf( buf, "%s, how much zeni do you wish to send to who?", ch->name );
      do_tell( banker, buf );
      return;
    }
    if ( str_cmp( arg2, "all" ) && !is_number( arg2 ) )
    ...
    if ( amount > ch->pcdata->balance )
    {
      sprintf( buf, "%s, you are very generous, but you don't have that much zeni!", ch->name );
      do_tell( banker, buf );
      return;
    }
    ...
    if ( amount <= 0 )
    {
      sprintf( buf, "Oh I see %s.. I didn't know I was doing business with a comedian.", ch->name );
      do_tell( banker, buf );
      return;
    }
    ...
    sprintf( buf, "You transfer %d zeni to %s's bank account.\n\r",
             amount, victim->name );
    set_char_color( AT_GREEN, ch );
    send_to_char( buf, ch );
    sprintf( buf, "%s just transferred %d zeni to your bank account.\n\r",
             ch->name, amount );
