  ...
  else if( !str_cmp( cmd, "show" ) && ( cmd_found = TRUE ) && table != NULL )
  {
    ...
    for( entry = table->first_entry, i = 1; entry; entry = entry->next, i++ )
    {
      if( !strcmp( table->keyword, "powerlevel" )
          ...
          || !strcmp( table->keyword, "plfierian" )
          || !strcmp( table->keyword, "plhylian" )
          || !strcmp( table->keyword, "plreploid" )
 ...
 * way. */
{
  ...
  for( num = 0; num < len; num++ )
    buf[num] = ' ';
  buf[num] = '\0';
  pager_printf_color( ch, "&z| |     &G%s%s%s%s     &z| |&w\n\r", buf, table->desc, odd ? " " : "", buf );
  ...
  for( entry = table->first_entry; entry; entry = entry->next )
  {
    if( !strcmp( table->keyword, "powerlevel" )
      ...
      pager_printf_color( ch, "&z| |               %s%2d. %-20.20s          &z| |\n\r",
                          num <= 3 ? "&R" : num <= 10 ? "&Y" : num <= 25 ? "&w" : "&p", num, capitalize( entry->name ) );
    else
      pager_printf_color( ch, "&z| |     %s%2d. %-20.20s%15s     &z| |\n\r",
  ...
  pager_printf_color( ch, "&z| |                                                 | |&w\n\r" );
  pager_printf_color( ch, "&z| |-------------------------------------------------| |&w\n\r" );
  pager_printf_color( ch, "&z(&r@&z)                                                 (&r@&z)&w\n\r" );
}
