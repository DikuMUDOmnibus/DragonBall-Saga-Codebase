...
void do_sit( CHAR_DATA * ch, char *argument )
{
  ...
  if( xIS_SET( ch->affected_by, AFF_FLYING ) ) {
     send_to_char("You must fly to the ground before you can take a rest.\r\n", ch);
     return;
  }
...
void do_sleep( CHAR_DATA * ch, char *argument )
{
  ...
  switch ( ch->position )
  {
    ...
    case POS_STANDING:
      ...
      /* See if they're flying.. If they're flying..  Let 'em drop to the ground */
      if( xIS_SET( ( ch )->affected_by, AFF_FLYING ) ) {
	send_to_char( "You pass out in mid-air and fall to the ground.\n\r", ch );
	xREMOVE_BIT( ( ch )->affected_by, AFF_FLYING );
	act(AT_ACTION, "$n passes out and mid-air falls down to the ground like a rock.", ch, NULL, NULL, TO_ROOM );
      } else {
